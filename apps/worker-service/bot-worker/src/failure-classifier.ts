...
export enum JoinFailureReason {
  ...
  BOT_ACCOUNT_LOCKED    = 'BOT_ACCOUNT_LOCKED',
...
export const FAILURE_TO_STATE: Record<JoinFailureReason, MeetingState> = {
...
const REJECTION_PATTERNS: Array<{
  ...
  /** If true, only classify as this reason if rejection happened quickly (< 3s). */
  quickOnly?: boolean;
}> = [
  {
...
// ─────────────────────────────────────────────────────────────────────────────
// Network-level 4xx signals captured from page.on('response')
// ─────────────────────────────────────────────────────────────────────────────
...
export class FailureClassifier {
  private _networkSignals: NetworkSignal[] = [];
  private _joinClickedAt: number | null = null;

  /** Call this when the user clicks "Ask to join" / "Join now". */
  markJoinClicked(): void {
   ...
   */
  async classify(page: Page): Promise<JoinFailureReason | null> {
    const url = page.url();
    ...
    if (has429) return JoinFailureReason.RATE_LIMITED;
  ...
  static describe(reason: JoinFailureReason): string {
    const descriptions: Record<JoinFailureReason, string> = {
      [JoinFailureReason.HOST_DENIED]:
        'The meeting host manually denied the bot entry.',
      [JoinFailureReason.DOMAIN_RESTRICTED]:
        'The Google Workspace admin policy blocks participants outside the organisation.',
      [JoinFailureReason.AUTH_REQUIRED]:
        'Google Meet requires a signed-in account. Bot session may be invalid.',
      [JoinFailureReason.SESSION_EXPIRED]:
        'The bot Google session has expired. Re-authentication required.',
      [JoinFailureReason.BOT_ACCOUNT_LOCKED]:
        'The bot Google account has been suspended or blocked.',
      [JoinFailureReason.RATE_LIMITED]:
        'Google is rate-limiting requests. The bot will cool down.',
      [JoinFailureReason.NETWORK_ERROR]:
        'Could not connect to Google Meet (DNS / network / timeout).',
      [JoinFailureReason.MEETING_NOT_FOUND]:
        'The meeting link is invalid or the meeting does not exist.',
      [JoinFailureReason.MEETING_ENDED_EARLY]:
        'The meeting ended before the bot could join.',
      [JoinFailureReason.UNKNOWN]:
        'An unclassified error occurred. Check bot logs for details.',
    };
