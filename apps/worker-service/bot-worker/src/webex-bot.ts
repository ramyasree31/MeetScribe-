...
function isSignInPage(url: string): boolean {
  return (
    url.includes('idbroker.webex.com') ||
...
async function saveScreenshot(page: import('playwright').Page, filename: string): Promise<void> {
  try {
    ...
    console.error(`[webex-bot] Failed to save screenshot "${filename}":`, (err as Error).message);
  }
}
...
export async function runWebexBot() {
  ...
  try {
    ...
    console.error('[webex-bot] Navigation failed:', (err as Error).message);
  ...
  try {
    ...
    console.error('[webex-bot] Timed out waiting to join:', (err as Error).message);
  ...
  if (wsReady || ws.readyState === WebSocket.CONNECTING) {
    ...
    if (ws.readyState === WebSocket.OPEN) {
      ...
      ffmpeg.stderr.on('data', (d: Buffer) => {
        ...
        if (line) console.log('[ffmpeg-webex]', line);
      });
  ...
  const checkInterval = setInterval(async () => {
    try {
      const leaveVisible =
        await page.isVisible('[aria-label*="leave" i]').catch(() => false) ||
      ...
      if (!leaveVisible) {
        ...
        if (!stillIn) await handleWebexEnd(browser, ws, meetingId, ffmpeg);
      }
    } catch (_) {
      clearInterval(checkInterval);
...
async function tryWebexGuestBypass(page: import('playwright').Page, meetingUrl: string): Promise<boolean> {
  const guestLabels = [
    /continue as guest/i,
    /join as guest/i,
  ...
  for (const label of guestLabels) {
    try {
      ...
      if (await btn.isVisible({ timeout: 3000 })) {
        await btn.click();
        await sleep(2000);
        if (!isSignInPage(page.url())) return true;
      }
    } catch (_) {}
  }
