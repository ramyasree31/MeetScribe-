...
async function saveScreenshot(page: import('playwright').Page, filename: string): Promise<void> {
  try {
    ...
    console.error(`[zoom-bot] Failed to save screenshot "${filename}":`, (err as Error).message);
  }
}
...
export async function runZoomBot() {
  ...
  try {
    ...
    console.error('[zoom-bot] Navigation failed:', (err as Error).message);
  ...
  if (isSignInPage(page.url())) {
    console.error(
      '[zoom-bot] Redirected to Zoom sign-in. ' +
  ...
  // ─── Click Join ───────────────────────────────────────────────────────────
  const joinSelectors = [
    { role: 'button' as const, name: /^join$/i },
    { role: 'button' as const, name: /join meeting/i },
    { role: 'button' as const, name: /join now/i },
  ];
  ...
  try {
    ...
    console.error('[zoom-bot] Timed out waiting to join:', (err as Error).message);
  ...
  if (wsReady || ws.readyState === WebSocket.CONNECTING) {
    ...
    if (ws.readyState === WebSocket.OPEN) {
      ...
      ffmpeg.stderr.on('data', (d: Buffer) => {
        ...
        if (line) console.log('[ffmpeg-zoom]', line);
      });
  ...
  const checkInterval = setInterval(async () => {
    try {
      ...
      if (leaveCount === 0 && muteCount === 0) {
        ...
        if (!stillIn) await handleZoomEnd(browser, ws, meetingId, ffmpeg);
      }
    } catch (_) {
      clearInterval(checkInterval);
      await handleZoomEnd(browser, ws, meetingId, ffmpeg);
